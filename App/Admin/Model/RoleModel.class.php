...
class RoleModel extends Model 
{
	...
	public function search($pageSize = 20)
	{
		...
		$data['page'] = $page->show();
		/************************************** 取数据 ******************************************/
		/*SELECT a.*,GROUP_CONCAT(c.pri_name) pri_name FROM sh_role a LEFT JOIN
sh_role_privilege b ON a.id=b.role_id LEFT JOIN sh_privilege c ON b.pri_id
=c.id GROUP BY a.id;*/
		$data['data'] = $this->field('a.*,GROUP_CONCAT(c.pri_name) pri_name')->alias('a')->join('LEFT JOIN sh_role_privilege b ON a.id=b.role_id LEFT JOIN sh_privilege c ON b.pri_id=c.id')->where($where)->group('a.id')->limit($page->firstRow.','.$page->listRows)->select();
		return $data;
	}
	// 添加前
